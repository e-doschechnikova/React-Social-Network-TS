...
const SidebarReducer = (state: SidebarFriendsStateType = initialState, action: any) => {
    return state;
};

///----------- types -----------\\\
type SidebarforFriendsStateType = {
    id: string;
    name: string;
    avatar: string;
};
export type SidebarFriendsStateType = {
    sidebarForFriends: Array<SidebarforFriendsStateType>;
};
